...
\chapter{Basic Data Science and Data Wrangling Knowledge}
...
\subsection{Model Selection and Overfitting Resolve}
...
$$L(w|x) = \prod_{i=1}^N[p(y=1|x,w)]^{y_i}[1-p(y=1|x,w)]^{1-y_i}$$
Take Log to get the Loss Function 
$$log L(w|x) = \sum_{i]1}^N -y_ilogy_i+ (1-y_i)log(1-y_i)$$
$$log L(w|x) = \sum_{i]1}^N y_iw^Tx - log(1+e^{w^Tx})$$
...
In the hidden layer, use activation function as activation fdunction 
