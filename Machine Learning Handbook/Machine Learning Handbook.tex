...
\chapter{Bayesian Probability Theory}

\section{Basic Concepts}

\begin{itemize}
	\item Bayesian Theorem: $$P(\theta|X) = \frac{P(X|\theta) P(\theta)}{P(X)}$$
		\subitem $P(\theta|X)$ is posterior probability
		\subitem $P(\theta)$ is prior
		\subitem $P(X|\theta)$ is likelihood
		\subitem $P(X)$ is evidence
	...
	\item Marginalization: $$P(X) = \int p(X,Y) dY$$
	\item Bayesian network
	\subitem $$P(X_1,...,X_n) = \prod_{k=1}^n P(X_k | Pa(X_k))$$
   	\subitem Plate notation
   	\item Analytical Inference
   	\subitem Maximum a Posterior (MAP) Estimation
   	\item \textbf{Conjugate Distribution}:
...
In State $\omega$ we take action $a \in A$, incur Loss $L(\omega,a)$, how to choose a to minimize
Risk:
...
\begin{itemize}
	\item Latent Variable Models
		\subitem Pros: simpler model(network), few parameters, sometimes has meaningful explanations
		\subitem Cons: harder to work with
		\subitem handle missing data, model missing data as latent variable (distribution). eg \textit{Probabilistic PCA}
	\item Expectation - Maximization (EM) Algorithm
		\subitem 
			\subsubitem E- Step: calculate latent variable distribution
				$$q(t_i) = p(t_i|x_i, \theta)$$
				for each data point
			\subsubitem M- Step: $$p(t_i|x_i, \theta)$$
			\subsubitem E-M: $$max_{\theta} \E_{q} log p(X, T|\theta)$$
		\subitem Example: Training Gaussian Mixture Model
...
\begin{itemize}
	...
	\begin{itemize}
    	...
    	\item encoder + decoder(x RNN connects to y RNN in sequence
  		\item many-to-one RNN (only one output at t)
